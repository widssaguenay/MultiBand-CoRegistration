...
LocalMatrix::LocalMatrix(const int32_t m_,const int32_t n_) {
  allocateMemory(m_,n_);
}

LocalMatrix::LocalMatrix(const int32_t m_,const int32_t n_,const FLOAT* val_) {
  allocateMemory(m_,n_);
  int32_t k=0;
...
LocalMatrix::LocalMatrix(const LocalMatrix &M) {
...
LocalMatrix& LocalMatrix::operator= (const LocalMatrix &M) {
  if (this!=&M) {
    if (M.m!=m || M.n!=n) {
      releaseMemory();
      allocateMemory(M.m,M.n);
...
void LocalMatrix::getData(FLOAT* val_,int32_t i1,int32_t j1,int32_t i2,int32_t j2) {
  if (i2==-1) i2 = m-1;
  if (j2==-1) j2 = n-1;
  int32_t k=0;
...
LocalMatrix LocalMatrix::getMat(int32_t i1,int32_t j1,int32_t i2,int32_t j2) {
  if (i2==-1) i2 = m-1;
  if (j2==-1) j2 = n-1;
...
void LocalMatrix::setMat(const LocalMatrix &M,const int32_t i1,const int32_t j1) {
...
void LocalMatrix::setVal(FLOAT s,int32_t i1,int32_t j1,int32_t i2,int32_t j2) {
  if (i2==-1) i2 = m-1;
  if (j2==-1) j2 = n-1;
  if (i2<i1 || j2<j1) {
...
void LocalMatrix::setDiag(FLOAT s,int32_t i1,int32_t i2) {
  if (i2==-1) i2 = min(m-1,n-1);
  for (int32_t i=i1; i<=i2; i++)
    val[i][i] = s;
}

void LocalMatrix::zero() {
  setVal(0);
}

LocalMatrix LocalMatrix::extractCols (vector<int> idx) {
	LocalMatrix M(m,idx.size());
  for (int32_t j=0; j<M.n; j++)
    if (idx[j]<n)
...
LocalMatrix LocalMatrix::eye (const int32_t m) {
	LocalMatrix M(m,m);
  for (int32_t i=0; i<m; i++)
    M.val[i][i] = 1;
  return M;
...
LocalMatrix LocalMatrix::diag (const LocalMatrix &M) {
  if (M.m>1 && M.n==1) {
	  LocalMatrix D(M.m,M.m);
    for (int32_t i=0; i<M.m; i++)
      D.val[i][i] = M.val[i][0];
    return D;
  } else if (M.m==1 && M.n>1) {
	  LocalMatrix D(M.n,M.n);
...
LocalMatrix LocalMatrix::reshape(const LocalMatrix &M,int32_t m_,int32_t n_) {
  if (M.m*M.n != m_*n_) {
    cerr << "ERROR: Trying to reshape a matrix of size (" << M.m << "x" << M.n <<
            ") to size (" << m_ << "x" << n_ << ")" << endl;
    exit(0);
  }
  LocalMatrix M2(m_,n_);
...
LocalMatrix LocalMatrix::rotMatX (const FLOAT &angle) {
  FLOAT s = sin(angle);
  FLOAT c = cos(angle);
  LocalMatrix R(3,3);
  R.val[0][0] = +1;
...
LocalMatrix LocalMatrix::rotMatY (const FLOAT &angle) {
  FLOAT s = sin(angle);
  FLOAT c = cos(angle);
  LocalMatrix R(3,3);
  R.val[0][0] = +c;
...
LocalMatrix LocalMatrix::rotMatZ (const FLOAT &angle) {
  FLOAT s = sin(angle);
  FLOAT c = cos(angle);
  LocalMatrix R(3,3);
  R.val[0][0] = +c;
...
LocalMatrix LocalMatrix::operator+ (const LocalMatrix &M) {
  const LocalMatrix &A = *this;
  const LocalMatrix &B = M;
  if (A.m!=B.m || A.n!=B.n) {
    cerr << "ERROR: Trying to add matrices of size (" << A.m << "x" << A.n <<
        ") and (" << B.m << "x" << B.n << ")" << endl;
    exit(0);
  }
  LocalMatrix C(A.m,A.n);
  for (int32_t i=0; i<m; i++)
    for (int32_t j=0; j<n; j++)
...
LocalMatrix LocalMatrix::operator* (const FLOAT &s) {
	LocalMatrix C(m,n);
  for (int32_t i=0; i<m; i++)
    for (int32_t j=0; j<n; j++)
      C.val[i][j] = val[i][j]*s;
...
LocalMatrix LocalMatrix::operator- () {
	LocalMatrix C(m,n);
  for (int32_t i=0; i<m; i++)
    for (int32_t j=0; j<n; j++)
      C.val[i][j] = -val[i][j];
...
LocalMatrix LocalMatrix::cross (const LocalMatrix &a, const LocalMatrix &b) {
...
LocalMatrix LocalMatrix::inv (const LocalMatrix &M) {
  if (M.m!=M.n) {
    cerr << "ERROR: Trying to invert matrix of size (" << M.m << "x" << M.n << ")" << endl;
    exit(0);
  }
  LocalMatrix A(M);
  LocalMatrix B = eye(M.m);
  B.solve(A);
  return B;
}

bool LocalMatrix::inv () {
  if (m!=n) {
...
bool LocalMatrix::solve (const LocalMatrix &M, FLOAT eps) {
  
  // substitutes
  const LocalMatrix &A = M;
  LocalMatrix &B       = *this;
...
bool LocalMatrix::lu(int32_t *idx, FLOAT &d, FLOAT eps) {
...
void LocalMatrix::svd(LocalMatrix &U2, LocalMatrix &W, LocalMatrix &V) {

  LocalMatrix U = LocalMatrix(*this);
  U2 = LocalMatrix(m,m);
  V  = LocalMatrix(n,n);
...
ostream& operator<< (ostream& out,const LocalMatrix& M) {
  if (M.m==0 || M.n==0) {
    out << "[empty matrix]";
  } else {
    char buffer[1024];
...
void LocalMatrix::allocateMemory (const int32_t m_,const int32_t n_) {
  m = abs(m_); n = abs(n_);
  if (m==0 || n==0) {
    val = 0;
    return;
...
void LocalMatrix::releaseMemory () {
  if (val!=0) {
    free(val[0]);
    free(val);
  }
...
FLOAT LocalMatrix::pythag(FLOAT a,FLOAT b) {
  FLOAT absa,absb;
  absa = fabs(a);
  absb = fabs(b);
  if (absa > absb)
